...
const Products = () => {
  ...
  const categories = ['sneaker', 'running', 'classic', 'lifestyle', 'skate'];
  ...
  return (
    <div className="container page">
      ...
      <form className="filter-bar" onSubmit={handleSearch}>
        ...
        <button type="button" className="btn btn-outline btn-sm" onClick={() => setFilters({ search: '', brand_id: '', min_price: '', max_price: '', category: '', page: 1 })}>
          Xóa lọc
        </button>
      </form>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 24, flexWrap: 'wrap' }}>
        <button
          className={`btn btn-sm ${!filters.brand_id ? 'btn-primary' : 'btn-outline'}`}
          onClick={() => setFilters(f => ({ ...f, brand_id: '', page: 1 }))}>
          Tất cả
        </button>
        {brands.map(b => (
          <button
            key={b.id}
            className={`btn btn-sm ${filters.brand_id == b.id ? 'btn-primary' : 'btn-outline'}`}
            onClick={() => setFilters(f => ({ ...f, brand_id: b.id, page: 1 }))}>
            {b.name}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="empty-state">
          ...
          <button className="btn btn-primary" onClick={() => setFilters({ search: '', brand_id: '', min_price: '', max_price: '', category: '', page: 1 })}>
            Xem tất cả
          </button>
        </div>
      ) : (
